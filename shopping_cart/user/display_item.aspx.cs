...
public partial class user_display_item : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            cmd.CommandText = "select * from product where product_category = '"+ Request.QueryString["category"] + "' ";

        }

        if(Request.QueryString["category"] == null && Request.QueryString["search"] != null)
        {
            cmd.CommandText = "select * from product where product_name like('%" + Request.QueryString["search"] + "%')";
